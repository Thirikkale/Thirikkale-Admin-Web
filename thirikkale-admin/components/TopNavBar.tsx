...
export function TopNavBar({ title = "Manager Dashboard", subtitle = "Welcome back! Here's what's happening today." }: TopNavBarProps) {
    ...
    const getDashboardName = (userType: string) => {
        switch (userType) {
            ...
            case "RiderSupport":
                return "Rider Support Dashboard"
            case "DriverSupport":
                return "Driver Support Dashboard"
            default:
                return "Manager Dashboard"
        }
    }
    ...
    return (
        <header className="flex h-16 shrink-0 items-center justify-between gap-4 border-b border-gray-200 bg-white px-6 shadow-sm transition-[width,height] ease-linear group-has-data-[collapsible=icon]/sidebar-wrapper:h-12">
            {/* Left side - Sidebar toggle and title */}
            <div className="flex items-center gap-4">
                <SidebarTrigger className="-ml-1 hover:bg-gray-100 rounded-md p-2" />
                <div className="flex flex-col">
                    <h1 className="text-xl font-semibold text-gray-900">{title || dashboardName}</h1>
            ...
            <div className="flex items-center gap-3">
                ...
                <div className="relative hidden md:block">
                    ...
                    <Input
                        placeholder="Search..."
                        className="w-72 pl-10 h-9 bg-gray-50 border-gray-200 focus:bg-white focus:border-blue-300 focus:ring-1 focus:ring-blue-200"
                    />
                </div>
                ...
                {/* Settings */}
                <Button variant="ghost" size="icon" className="hover:bg-gray-100 h-9 w-9">
                ...
                <DropdownMenu>
                    <DropdownMenuTrigger asChild>
                        <Button variant="ghost" className="flex items-center gap-3 px-3 py-2 hover:bg-gray-100 rounded-lg h-10">
                            <div className="flex items-center gap-3">
