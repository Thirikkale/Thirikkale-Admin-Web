...
const DriverToCompanyPayouts = () => {
    // Mock payouts: drivers owe company (cash collected minus commission)
    const payouts = [
        {
            id: "PAY001",
            driverId: "DRV123",
    ...
    // Calculate Amount Owed to Company (cash collected minus commission)
    const payoutsWithAmount = payouts.map(p => ({
        ...p,
        amountOwed: p.cash - p.commission
    }))
    ...
    // Metrics
    const totalOwed = payoutsWithAmount.reduce((sum, p) => sum + p.amountOwed, 0)
    ...
    // removed unused completedCount and pendingCount
                    ...
                    {filteredPayouts.map((payout) => (
                        <div key={payout.id} className="border-b border-gray-200 bg-white hover:bg-gray-50 transition-colors">
                            <div className="grid gap-4 px-6 py-4 items-center" style={{ gridTemplateColumns: '1fr 1.5fr 1.2fr 0.4fr 1fr 1fr 1fr 0.9fr 0.9fr' }}>
                                ...
                                {/* Owed to Company */}
                                <div className="text-right text-xs font-bold text-blue-900">LKR {payout.amountOwed.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</div>
                                {/* Status */}
