...
import {
    getAllPayments,
    getAllDrivers,
    calculateCompanyToDriverPayouts,
    type CompanyToDriverPayout
} from "@/lib/api/adminService"
import { Loader } from "@/components/ui/loader"

const CompanyToDriverPayouts = () => {
    const [payouts, setPayouts] = useState<CompanyToDriverPayout[]>([])
    ...
    const fetchPayoutData = async () => {
        ...
        try {
            ...
            if (paymentsResponse.data && driversResponse.data) {
                // Calculate company to driver payout summaries
                const payoutSummaries = calculateCompanyToDriverPayouts(
    ...
    // Calculate metrics (CARD ONLY)
    const totalAmountToPay = payouts.reduce((sum, p) => sum + p.amountToPayDriver, 0)
    const totalCommission = payouts.reduce((sum, p) => sum + p.totalCommission, 0)
    const totalCard = payouts.reduce((sum, p) => sum + p.totalCardPayments, 0)
    ...
    const metrics = {
        ...
        commission: {
            ...
            change: '+7.8%',
            trend: 'up',
            period: `${payouts.length > 0 ? ((totalCommission / totalCard) * 100).toFixed(1) : 15}% avg rate`
        },
        card: {
            value: `LKR ${totalCard.toLocaleString(undefined, { minimumFractionDigits: 2 })}`,
            change: '+12.1%',
            trend: 'up',
            period: 'total card revenue'
        },
        payouts: {
                ...
                <>
                    ...
                    <div className="bg-white border border-gray-200 shadow-sm rounded-lg overflow-hidden">
                        ...
                        <div className="w-full">
                            {/* Table Header - Card Only (No Cash column) */}
                            <div className="bg-gray-100 border-b-2 border-gray-300">
                                <div className="grid gap-4 px-6 py-4" style={{ gridTemplateColumns: '0.7fr 1.5fr 1fr 0.5fr 1fr 1fr 1fr 0.8fr 0.8fr' }}>
                            ...
                            {loading ? (
                                ...
                                <div key={payout.driverId} className="border-b border-gray-200 bg-white hover:bg-gray-50 transition-colors">
                                    <div className="grid gap-4 px-6 py-4 items-center" style={{ gridTemplateColumns: '0.7fr 1.5fr 1fr 0.5fr 1fr 1fr 1fr 0.8fr 0.8fr' }}>
                                        {/* Payout ID */}
                                        ...
                                        {/* Card Revenue (only card, no cash) */}
                                        <div className="text-right text-xs text-purple-700 font-semibold">LKR {payout.totalCardPayments.toLocaleString(undefined, { minimumFractionDigits: 2 })}</div>
                                        {/* Commission (15% of card only) */}
                                        ...
                                        {/* Amount to Pay (card - commission) */}
                                        <div className="text-right text-xs font-bold text-blue-900">LKR {payout.amountToPayDriver.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</div>
                                        {/* Status */}
